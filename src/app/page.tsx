    ...
    <div className="h-screen flex flex-col overflow-hidden bg-white dark:bg-gray-950">
      ...
      <header className="h-12 flex items-center justify-between px-4 bg-emerald-600 shrink-0 z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-emerald-200 text-xs ml-1 hidden md:block">· Water Purifier</span>
        </div>
        <div className="flex items-center gap-2">
          {/* Bulk Messaging Link */}
          <Link
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className={`flex flex-col overflow-hidden border-r border-gray-200 dark:border-gray-800 ${mobileView === 'list' ? 'flex' : 'hidden'} md:flex md:w-80 md:shrink-0 w-full`}>
          <ConversationList selectedId={selected?.id ?? null} onSelect={handleSelect} onDelete={handleDelete} />
        </div>
        <div className={`flex-1 flex flex-col overflow-hidden min-w-0 ${mobileView === 'chat' ? 'flex' : 'hidden'} md:flex`}>
          <ChatWindow
            conversation={selected}
            onAIToggle={(id, mode) => {
              if (selected?.id === id) setSelected((prev) => prev ? { ...prev, ai_mode: mode } : null)
            }}
          />
        </div>
        <div className={`flex flex-col overflow-hidden border-l border-gray-200 dark:border-gray-800 ${mobileView === 'lead' ? 'flex' : 'hidden'} md:flex md:w-72 md:shrink-0 w-full`}>
          <LeadPanel conversation={selected} lead={lead} onLeadUpdate={handleLeadUpdate} />
        </div>
      </div>
      ...
      {selected && (
        <div className="md:hidden flex border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 shrink-0">
          {(['list', 'chat', 'lead'] as MobileView[]).map((view) => (
            <button
              key={view}
              onClick={() => setMobileView(view)}
              className={`flex-1 py-3 text-xs font-medium transition-colors capitalize ${
                mobileView === view ? 'text-emerald-600 border-t-2 border-emerald-500' : 'text-gray-500'
              }`}
            >
              {view === 'list' ? 'Inbox' : view === 'chat' ? 'Chat' : 'Lead Info'}
            </button>
          ))}
        </div>
      )}
